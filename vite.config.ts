...
import { speechmaticsApiPlugin } from './server/speechmatics-api.ts';
...
export default defineConfig(({ mode }) => {
  const env = loadEnv(mode, process.cwd(), ['TYPESAFE_', 'JEV_', 'SPEECHMATICS_', 'OPENAI_']);
  assertJevContext(env.JEV_CONTEXT_VERSION);
  return {
    plugins: [jevApiPlugin({ apiKey: env.TYPESAFE_API_KEY, model: env.JEV_MODEL }),
      speechmaticsApiPlugin({ apiKey: env.SPEECHMATICS_API_KEY }),
