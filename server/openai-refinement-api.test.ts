...
it('status and upstream use overrides, validate input, and never expose keys', async () => {
  const transport = vi.fn().mockResolvedValue(Response.json({ status: 'completed', output: [
    { type: 'message', content: [{ type: 'output_text', text: JSON.stringify({ displayText: 'Clear text.' }) }] },
  ] }));
  ...
  expect(JSON.parse(transport.mock.calls[0]![1].body).model).toBe('model-test');
